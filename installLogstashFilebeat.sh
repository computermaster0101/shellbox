...
function -h {
cat <<USAGE
USAGE: installLogstashFilebeat.sh (presented with defaults)
                             (--version "2.1")?
                             (--logstashServer "localhost")?
                             (--logstashPort 9210)?
                             (--serverName "")?
                             (--tag "")?
                             (--specificLogFile "/var/log/*.log")? (optional) Must provide a fully qualified log file.

  Install and configure Logstash Filebeat.
...
function options {
  ...
  do
    case "$1" in
      ...
      --tag)                tag="$2"                 ; shift ;;
      --specificLogFile)    specificLogFile="$2"     ; shift ;;
...
function validate {
    ...
    if [ -z ${logstashServer+x} ]; then
        logstashServer="localhost"
        echo "** logstashServer not set, using default --> ${logstashServer}"
    fi
    if [ -z ${logstashPort+x} ]; then
        logstashPort="9210"
        echo "** logstashPort not set, using default --> ${logstashPort}"
    fi
...
EOF
    ...
    if [ ! -z ${tag+x} ]; then
        sed -i'.bak' "s|  tags: .*|  tags: [\"filebeat\",\"${tag}\"]|" ${home}/filebeat.yml
        rm ${home}/filebeat.yml.bak
    fi

    ## if we have a specificLogFile, as it is optional
    if [ ! -z ${specificLogFile+x} ]; then
        sed -i'.bak' "s|        - /var/log/\*\.log|        - ${specificLogFile}|" ${home}/filebeat.yml
        rm ${home}/filebeat.yml.bak
    fi
    ...
    ## install service
    sudo bash ${CURRENT_DIR}/installSupervisordConfig.sh --name "filebeat" --toexecute "filebeat" directory="${home}" --serviceuser "root"
